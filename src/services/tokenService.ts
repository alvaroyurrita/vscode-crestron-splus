import { ExtensionContext, Disposable, Uri, Position, CompletionItemKind, CompletionItem, SnippetString, CompletionItemLabel } from "vscode";
...
export class TokenService implements Disposable {
    ...
    public getDocumentMembers(uri: Uri): DocumentToken[] | undefined {
        ...
        const documentTokens = this._documentTokenService.getTokens(uri);
    ...
    public getBlockStatementTokenAtPosition(uri: Uri, position: Position): DocumentToken | undefined {
        ...
        documentMember = documentMembers.
            filter(member => !member.isExternalLibrary).
            find(member => member.blockRange?.contains(position) ?? false);
